...
class WorkerRequest(BaseModel):
    ...
    model: str = "MDX23C"  # Modèle UVR (ex: MDX23C, VR, Demucs...)
...
def run_worker(request: WorkerRequest, credentials: HTTPBasicCredentials = Depends(check_auth)):
    ...
    output_dir = f"/output/{request.file}_uvr"

    # Vérifier si le fichier existe
    if not os.path.exists(input_file):
        raise HTTPException(status_code=400, detail="Le fichier spécifié n'existe pas")

    # Exécuter Ultimate Vocal Remover via subprocess
    try:
        result = subprocess.run([
            "uvr", "--input", input_file, "--model", request.model, "--output", output_dir
        ], capture_output=True, text=True, check=True)

        return {"message": "Traitement terminé", "file": request.file, "output_dir": output_dir, "logs": result.stdout}

    except subprocess.CalledProcessError as e:
        raise HTTPException(status_code=500, detail=f"Erreur lors de l'exécution de UVR : {e.stderr}")
